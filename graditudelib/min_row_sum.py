...
def exclude_the_min_row_sum(feature_count_table,
                            feature_count_start_column, min_row):
    ...
    min_row_sum(matrix_value_with_gene_name, colum_with_gene_name, min_row)
...
def min_row_sum(value_matrix, gene_column, min_row):
    gene_table_final = []
    combined_df_ext = pd.concat([gene_column, value_matrix], axis=1)
    ...
    df_with_min_row_samples.to_csv('', sep='\t')
