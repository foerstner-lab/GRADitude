...
def correlation_specific_gene_against_all(feature_count_table, feature_count_start_column, feature_count_end_column,
    ...
    gene_row_specific_gene = _extract_gene_row(table_with_genes_new_index, name)
    correlation_series_against_dataframe(gene_row_specific_gene, table_with_genes_new_index, output_file)
...
def correlation_series_against_dataframe(gene_row_specific_gene, table_with_genes_new_index, output_file):
    rho_list = []
    p_value_list = []
    for index_gene, row_gene in table_with_genes_new_index.iterrows():
        rho = spearmanr(gene_row_specific_gene, row_gene, axis=None)[0]
        rho_list.append(rho)
        p_value = spearmanr(gene_row_specific_gene, row_gene, axis=None)[1]
    ...
    table_with_rho_and_pvalue.columns.values[[-1, -2]] = ['rho_spearman_correlation', 'p_value']
