...
def _extract_gene_matrix(feature_count_table_df):
    gene_column = feature_count_table_df[list(filter(
        lambda col: col.startswith("Statistic"), feature_count_table_df.columns))]
    return gene_column
...
def min_row_sum_ercc_table(table_filtered_ercc, colum_with_libraries, min_row_sum):
    gene_table_final = []
    grad_cols = [col for col in table_filtered_ercc.columns if col.startswith('Grad')]
    summed_values = table_filtered_ercc[grad_cols].sum(axis=1)
