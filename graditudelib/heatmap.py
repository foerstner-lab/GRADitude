...
def plot_heatmap(feature_count_table, feature_count_start_column, feature_count_end_column, y_label_, output_file):
    table = pd.read_table(feature_count_table, sep=",")
    table.set_index('Gene', inplace=True)
    value_matrix = _extract_value_matrix(table, feature_count_start_column, feature_count_end_column)
